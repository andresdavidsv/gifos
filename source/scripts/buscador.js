...
search_input[0].addEventListener("keyup", (evento) => {
  if (evento.keyCode == key_code_enter) {
    ...
    let url_autocomplete = "https://api.giphy.com/v1/gifs/search/tags?" + q + "?&" + APIKEY;
...
let searchGifo = () => {
  ...
  let q = "q=" + search_input[0].value;
  let url_search_gifs = "https://api.giphy.com/v1/gifs/search?" + q + "?&" + APIKEY;
...
fetch(url_tags)
  ...
  }).catch(mesagge_error => console.error(mesagge_error));
